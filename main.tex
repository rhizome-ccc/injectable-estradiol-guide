...
\begin{titlepage}
  ...
  \setcounter{page}{0}
  \begin{center}
    \includegraphics[width=8em]{estradiol} \\
    \vspace{3em}
...
\begin{center}
  ...
  \parbox{0.8\textwidth}{
    \small
    This guide is released into the public domain; all rights to the
    work worldwide under copyright law, including all related and
    neighboring rights, are waived to the extent allowed by law.
  } \vfill
\end{center}
...
professional if this is possible.

With that in mind, if any information in this booklet is found to be
inaccurate or outdated, or if you have some additional information you
think should be included, please let us know so that we can update it
and inform others.  See the back of this booklet for a link to the git
repository for this guide, where you can report issues or suggest
improvements.

\section{Bridging Prescriptions}

If you have access to an NHS GP and are on the waiting list for an NHS
Gender Identity Clinic, you may be able to get a \textit{bridging
  prescription} for gender-affirming hormone therapy before your
initial appointment at the GIC.  This should be regarded as a superior
option to self-medicating with grey/black market medications, as your
doctor will then provide all the recommended monitoring and the
medication you receive will be far more tightly regulated.

However, the vast majority of GPs will be extremely hesitant to
provide a bridging prescription.  You can maximise your chances of
getting one by stressing the following points:

\begin{itemize}
\item You will be / have already started self-medicating with black
  market hormones
\item Waiting to be seen by a gender clinic is having a serious
  negative effect on your mental health
\item They should provide a bridging prescription as a \textbf{harm
    reduction} measure
\end{itemize}

You can also request to see a different doctor or change GPs if your
first attempt is refused.

It is important to note, however, that getting a bridging prescription
may prove infeasible for many people, and indeed may not be an option
at all for anyone without access to an NHS GP.  The rest of this guide
therefore focuses on providing you with the information you need to
self-medicate as safely as possible.
...
testosterone production when at suitably high levels.  This is called
estradiol monotherapy and, since it avoids the additional side effects
and separate risk profile of an anti-androgen medication and only
involves sourcing one medication rather than two, it is generally the
route taken for those self-medicating.
...
The effects of monotherapy are less studied than those of
administration of an anti-androgen as well as oestrogen and NHS Gender
Identity Clinics generally prescribe an anti-androgen along with
oestrogen for this reason.  Typically, this is a GnRH agonist such as
leuprorelin or triptorelin as these are highly effective and have a
favourable side effect profile in comparison to other anti-androgens.
However, for those self-medicating, accessing these medications can be
highly challenging as they typically are very expensive and hard to
source.  It is therefore more common to find other anti-androgens such
as cyproterone acetate or finasteride being used, though both of these
come with many side effects and contraindications.

\section{Dosage}

Injectable oestrogens are not typically estradiol (which is the actual
hormone you would find in the blood serum), but rather
\textit{estradiol esters}.  These are compounds which have minimal
effects themselves but are slowly metabolised into estradiol once in
the body.  This means that the oestrogen is ``released'' slowly, and
thus administration can be less frequent.  If estradiol itself were
used, as is the case for transdermal gels or patches, administration
would have to be daily to maintain stable levels.
...
enanthate (EEn), which is \textbf{0.11 mL of 50 mg/mL oil, every 7
  days}.  If an anti-androgen is also being taken along with the
estradiol, this can be reduced to \textbf{0.07 ml every 7 days}, with
a lower target level of around 200 pg/mL.
...
EEn oil should be administered as an intramuscular (IM) injection.
This means the medication goes into muscle tissue, \textbf{not} into
the fat under the skin (subcutaneous) or a blood vessel (intravenous).
IM injections are extremely low risk as long as the appropriate steps
are taken to mitigate infection risk and a suitable injection site is
used.
...
\item 1 ml syringe with 27 gauge needle
...
\item Check for air bubbles in the syringe. Remove them if there are
  ...
  to the required volume and pull the needle out of the vial
...
\item Insert the needle at a 90\textdegree\ angle in one quick and
  firm motion and slowly push the plunger down until the syringe is
  empty.  Try to keep the muscle as relaxed as possible.
\item Remove the needle, discard it into the sharps bin and release
  the tension on the skin.
...
a syringe and injected -- this is not necessarily dangerous for small
particles but best avoided nonetheless.  Coring can also lead to a
hole forming in the stopper that is large enough for air, dust and
other contaminants to enter the vial, or for fluid to leak out when
the vial is not upright.
